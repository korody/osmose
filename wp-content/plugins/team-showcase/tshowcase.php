...
function build_tshowcase($orderby="menu_order",$limit=-1,$idsfilter="0",$exclude="0",$category="0",$url="inactive",$layout="grid",$style="float-normal",$display="photo,name,position,email",$pagination="false",$imgwo="",$searchact="true",$show_id="true",$relation="OR",$page=1,$id='0',$taxonomy='0',$atts=array()) {
...
function tshowcase_build_categories_filter($display,$category) {
	...
	$html = '';
      ...
      $html .= "<ul id='ts-isotope-filter-nav'>";
			...
			$html .= "<li id='ts-all' data-filter='*'>".$ts_labels['filter']['all-entries-label']."</li>";
			...
			$args['parent'] = 0;
			 ...
			 if ( $count > 0 ){		 
					 foreach ( $terms as $term ) {
					 	...
					 	if($children_count) {
					 		...
					 		foreach ( $children as $cterm ) {
					 			$childs .= "<li id='ts-".$cterm->slug."' data-filter='.ts-".$cterm->slug."'>".$cterm->name."</li>";
					 		}
					...
					$html .= "<li id='ts-".$term->slug."' data-filter='.ts-".$term->slug."'>".$term->name.$childs."</li>";
